...
Per-bug "Status (shipped)" callouts below indicate which findings have already been addressed in the bridge.
...
**Status (shipped):** Added a `Makefile` with `make build` and documented in README.
...
**Status (shipped):** Validate the target exists in `browser_list_tabs` before dispatching; return `{error_code: "invalid_args", message: "no active target"}` otherwise.
...
**Status (shipped):** Always decode and surface the unmarshal error explicitly. Return `nil` (with the error) if undecodable. Never return the raw string fallback.
...
**Status (shipped):** After `cdp.Dial`, issue `Browser.getVersion` and only return success if it round-trips. Reflect actual liveness in `meta_health`.
...
**Status (shipped):** `--project <name>` flag (auto-generated short ID if omitted), sidecar JSON tracking owned `target_id`s, project-filtered `browser_list_tabs` (with `include_all: true` opt-out), new tools `browser_adopt_tab` / `browser_release_tab` / `browser_list_projects`.
...
**Status (shipped):** Server-side defaults that make every call cheaper without per-call args:
...
**Status (shipped):** Three coordinate tools that pass through to the existing CDP path:
...
## Wishlist captured for v2

These came up during the integration but didn't fit the "fix what's broken + isolate projects" focus of the immediate iteration:
...
- ~~**Network request bodies in event payloads**~~ — shipped as `browser_get_recent_events {include_bodies: true}`.
...
These belong in a future iteration.
